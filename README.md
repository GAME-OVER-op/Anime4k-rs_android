...
- Интерфейс переделан ближе к Google Material 3: светлая тема, нижняя навигация, крупные карточки и чистые поля ввода.
- Добавлена вкладка **Сохранённые** — показывает результаты, которые лежат в памяти приложения.
- Для каждого результата доступны действия:
  - сохранить в галерею;
  - поделиться;
  - удалить из памяти приложения.
- Добавлена вкладка **Настройки**:
  - включить/выключить ползунки;
  - включить/выключить foreground service с уведомлением;
  - запросить разрешение на уведомления.

## Фоновая обработка

Во время апскейла приложение запускает foreground service и показывает уведомление со статусом прогресса. Это помогает Android понять, что задача активная и её не нужно убивать как обычный фоновой процесс.
...
Важно: Android всё равно может ограничивать очень долгие задачи, особенно при агрессивной экономии батареи. Но foreground service + уведомление — правильный Android-способ для долгой обработки.
